...
| `initBrain` | `{ repoPath, mode: "ingest" \| "layer1" \| "layer1+layer2", scope?: { areas: string[] }, budget? }` | `{ jobId }` scope limits Layer 2 to named areas (sub-75 path). Mode `layer1+layer2` is SPEND-TOUCHING: Layer 2 runs LLM narration on the user's engineer key, so the estimated budget is displayed and confirmed by an explicit user action before the job starts; without confirmation the call is refused with `-32050` [Amended 2026-08-22, the last deferral built: mode `ingest` runs the adopt pipeline - the curated knowledge folder (agent/, .daijin/brain, ai-brain/, brain/, .brain/; first with markdown wins) is split into units marked human-written, validateCitations is the mechanical drift check (a unit citing a file or commit that is not there is dropped and named in the report), the derived brain lands in `.daijin/brain` and the SOURCE FOLDER IS NEVER WRITTEN. Zero spend: no narration runs. A repo with no qualifying folder is refused `-32602` BEFORE the job starts, naming the fix] |
| `diagnose` | `{ repoPath, control?: boolean }` | `{ caseRate: { exact, cases }, violations, cases, hits, misses, perCase, clusters: { byType, byArea, byArm }, identifierMisses, recommendation, discriminatingRange, controlSkipped }` [Amended 2026-08-17, D-0035 live tier: this row was PROSE, and prose is the right level for a return that is "the file, plus classification" and the wrong level for a structured result carrying clusters, a case rate, violations and a range. It was also the method that exposed the tiering hole: listed as live because it embeds, described in prose so nothing could compare it, and therefore checked by NO tier while the gate printed it as covered elsewhere. `discriminatingRange` is null when the control arm was not run and carries `measuredAt`, `fresh`, `stale` and `staleBecause` when it was, so a recalled measurement is never read as a fresh one; `controlSkipped` names why when the arm could not run. `recommendation` is null here by design: choosing between enriching docs, running Layer 2 or bootstrapping through the gym is the auditor's call, and that one spends] mechanical sub-75 diagnosis, zero-spend: missed gold cases clustered by type, area, and arm, from the retrieval-score tooling. [Addition 2026-08-16, v5, D-0030's surface: `control: true` (the LITERAL true; a truthy guess does not arm it) adds a permuted arm over the same store, embedder, k and budget, and returns `discriminatingRange: { caseRate: { candidate, control, range, casesOfHeadroom }, mrr: { candidate, control, range } }` or `controlSkipped` naming why (a corpus with under two distinct answers cannot be permuted). OFF by default because the arm doubles the call's wall clock; `discriminatingRange` is null when unmeasured, never zeroes, since a client must distinguish unmeasured from measured-and-found-nothing.] [Amended 2026-08-23, same screen-load cost: `recall: true` clusters over the stored last measurement (arithmetic plus a sqlite read, no embedder), stamped `recalled: true`; per-arm attribution is unattributed on recall because arms are not stored. No stored record falls through to a fresh measurement] |
...
| method | params | returns |
| --- | --- | --- |
| `gymStart` | `{ repoPath, config }` | `{ jobId }` REQUIRES the open spend gate; refused otherwise [Amended 2026-08-18, owner round 9: THE STUDENT DRIVER IS REAL. The engineer is built from the configured engineer role before the job starts (an unconfigured role is a -32602 at the call, not a failure inside a consented job): claude-code roles run one confined agentic CLI attempt in the sandbox (file tools only, no Bash - the harness owns running gates) with the owner's chosen sub-agent file as the system prompt, and API roles drive a strict JSON action protocol (read / edit / check / submit) where the DAEMON executes edits with the path confined to the sandbox. Gates default to the repo's own gates.yaml rows that carry signal (enabled, live or measured) when `config.gates` is absent, and a repo where no gate carries signal is refused with -32602 before consent is spent - the cycle's measurement IS baseline-vs-candidate gates. The job re-blocks the spend gate in its finally (D-0060)] [Amended 2026-08-18, owner round 10: THE TEACHER IS WIRED. On gradable modes (`experiment`, `evaluation`) the cycle grades each applied attempt inline: the TEACHER role's configured model receives the task, the submission diff, the gate results and the shown document ids (never the gold commit), and its rubric walks the full grading.js refusal list - five cited axes, verdict capped mechanically, author stamped from the configured identity, grader independence checked against the exam's recorded author - before importRubricBatch stores it beside the run. `harness-debug` stays UNGRADED because the ledger refuses rubrics for it; that boundary is respected, not tested. An unconfigured teacher role on a gradable mode refuses at the call, before consent buys a cycle whose grades can never arrive. A grading refusal mid-cycle is a `gym-grading-refused` step and the attempt stays pending (a rubric can be written later; an attempt cannot be re-run); it never fails the cycle] [Amended 2026-08-23, the all-exams draw: an OPEN draw (no `examId`) skips exams the draw rule would refuse - a reserved held-out exam is excluded from training sampling by asking examDrawRefusal itself, never by a duplicated rule - and `cohortSize` bounds how many are drawn. An EXPLICIT examId keeps its loud refusal. An open draw where everything is reserved refuses with "nothing drawable"] |
| `gymStatus` | `{ repoPath, jobId? }` | `{ cycles, activeRun?, ledger, harvest }` [Amended 2026-08-22, harvest wiring: `harvest` is the repo's harvest batches, newest first, SUMMARIES ONLY: `[{ id, cycleId, mode, at, questionsAsked, accepted, rejected, applied, appliedAt, written }]`. `applied` is a boolean and `appliedAt`/`written` are null until gymHarvestApply ran; an unapplied batch is visibly unapplied, which is the platform's seven-silent-cycles lesson made a wire fact. The full proposal set is not on this surface; it rides the harvest job's stream and the batch record] |
| `gymHarvest` | `{ repoPath, cycleId?, confirm: true }` | `{ jobId }` the LEARNING half of the loop: turn a graded cycle's gap tags into reviewed lesson proposals. SPEND-TOUCHING, under BOTH locks gymStart holds (owner gate scope `gym-cycle`, per-call consent): the teacher role answers one question per gap ("what knowledge, had it been retrieved, would have prevented this?") in a real provider generation. PROPOSAL-ONLY (P7 clause 10): the job validates every answer through harvest.js (an invented document id, a transposed pair, an unanswered question or an empty concern is a refusal), runs the curation gates and the current-code citation backstop, and records the surviving batch in the ledger; NO brain document is written by this method. `cycleId` defaults to the newest completed graded cycle; a harness-debug cycle is refused (never graded, nothing to learn), and a held-out run is refused before any question is asked. Zero accepted proposals is a recorded, measured outcome, not an error. The job ends `done`/`harvested` with `{ batch, questions, accepted, rejected }` counts, and the gate re-blocks in its finally (D-0060). [Added 2026-08-22, D-0065. Funnel and refusals pinned in gym-harvest-rpc.test.js] |
| `gymHarvestApply` | `{ repoPath, batchId, confirm: true }` | `{ jobId }` THE SEPARATE ACT (P7 clause 10): write an evaluation batch's accepted proposals into the repo's brain as lesson documents (`.daijin/brain/lessons/`, one file per lesson, id `gym-lesson-b<batch>-r<run>-g<gap>`, provenance in the body) and REINDEX the brain so retrieval serves what was learned. NOT spend-gated: no provider is called; this is the owner's hand, and `confirm: true` is still required because it writes into the repo's brain. The mode quarantine is enforced twice, in applyProposals and in the ledger: a non-`evaluation` batch is refused (an experiment batch is a recommendation to read), and a batch applies at most once. Targeted kinds (`retrieval-fix`, `sharpen-convention`) become lessons that NAME their target document rather than machine-edits of the user's own files. [Added 2026-08-22, D-0065] |
...
gate), `examMine` (both locks: the exam-mining gate scope and per-call consent), `gymHarvest` (both locks: the gym-cycle gate scope and per-call consent; the teacher answers one question per gap), `goalStart` ONLY with `triage: true` (the sweep itself is mechanical and free; auditor triage takes both locks before the loop starts, and the configured watcher role verifies each new finding under the same locks), `rolePing` (explicit user action per call), `initBrain` with mode
...
contract change, not an implementation detail. (`gymHarvestApply` calls no
provider and is NOT spend-touching; it still requires `confirm: true` because
it writes into the repo's brain.)
